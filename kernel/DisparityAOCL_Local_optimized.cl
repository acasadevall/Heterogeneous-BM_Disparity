...
__kernel void BM_Disparity_WorkGroup(
	global unsigned char* restrict left_im,
	global unsigned char* restrict right_im,
	global unsigned int* restrict disp_im,
	unsigned int MAX_D)
{
   	//int glIdy = get_global_id(0);
	//__attribute__((numbanks(1024), bankwidth(8)))
	...
	for (int glIdy=HALF_KERNEL; glIdy<(HEIGHT-HALF_KERNEL); glIdy++)
	{
		...
		for (int glIdx=HALF_KERNEL; glIdx<(WIDTH-HALF_KERNEL); glIdx++)
		{
			...
			DISP_IM(glIdy,glIdx,WIDTH) = disp;
		}
	}
